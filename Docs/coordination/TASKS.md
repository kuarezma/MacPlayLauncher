...
- **sahip (döngü):** Opus tasarım → Codex uygula/render → **Gemini 3.1 Pro görsel yargı** → Opus fix · **zeka: FAZ-BAZLI** (Faz A 🟡 · Faz B 🟠 · görsel yargı 🟠 · fix tasarımı 🔴) — bütün-task değil, fazına bak · **durum:** Faz C identity-remap ❌ → **Faz C.2 index-threshold ilk deneme ❌ (rider yok, paylaşılan-shader yan etkisi var)** → **Opus kararı bekliyor** · **kapsam:** `~/Cossacks3_Mac_Port` (repo DIŞI)
...
- **Faz C.2 (Codex GPT-5.4):** ✅ uygulandı. İlk threshold seti `b16>=8`, `b20>=10`, `b42>=30` ile render alındı. Geçersiz desktop-capture seti `20260628_213436` fokus bug'ıydı; harness düzeltildi ve geçerli oyun-içi set `20260628_213925` üretildi. Sonuç: rider eyere oturmadı; bazı remap'lenen birimler dik kolon/totem gibi bozuldu. → C.0 cavalry-only force-all fallback / kapatma kararı Opus'ta.
