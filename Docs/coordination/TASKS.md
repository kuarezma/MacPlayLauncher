...
- **sahip (döngü):** Opus tasarım → Codex uygula/render → **Gemini 3.1 Pro görsel yargı** → Opus fix · **zeka: FAZ-BAZLI** (Faz A 🟡 · Faz B 🟠 · görsel yargı 🟠 · fix tasarımı 🔴) — bütün-task değil, fazına bak · **durum:** Faz C ❌ → Faz C.2 ❌ → **Faz C.3 exact family/index ❌ (yan hasar yok, rider yine eyerde değil) → Opus kapanış kararı bekliyor** · **kapsam:** `~/Cossacks3_Mac_Port` (repo DIŞI)
...
- **Faz C.3 (Codex GPT-5.4):** ❌ ölçüm: cavalry `b16`, horse `index=0`, rider parçaları `{4,5,6,11,12,14,15}`. Yalnız `b16 index>=4 → bone[0]`; `b20/b42` orijinal. Karışık sahnede totem yan hasarı yok ama rider hâlâ eyerde değil. C.0 magenta passthrough gerçek yolu maskelemiş; forensik shader yolu tükendi → Opus kapanış.
- **çıktı:** `~/Cossacks3_Mac_Port/CAVALRY_FORENSIK_NOTU.md` güncellendi.
- **verify:** Faz C.3 geçerli ölçüm setleri: `20260630_230424` (`b42` değil), `20260630_234159` (`b16`, rider index bloğu); dar-fix karışık seti: `20260701_000119`. `oyna_ucretsiz.sh` SHA1 hâlâ `b18c446b17446a26ef9ca3a561eec34b6bb99624`; orijinal `wine_cx` dizinine dokunulmadı. NEXT: Opus kapanış kararı.
